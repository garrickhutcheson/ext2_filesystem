...
blk_iter *init_iter(blk_iter *it, minode *mip) {
  *it = (blk_iter){.nth = -1, .mip = mip};
  return it;
}
...
char *next_blk(blk_iter *it) {
  ...
  if (!it || !it->mip)
    return 0;
  it->nth++;
  if (it->nth < direct_end) {
    // direct blocks
    get_block(me, i_block[it->nth], it->buf);
  } else if (it->nth < indirect_end) {
    // indirect blocks
    if (it->nth == indirect_start)
      get_block(me, i_block[12], it->map1);
    get_block(me, it->map1[it->nth - indirect_start], it->buf);
  } else if (it->nth < double_end) {
    // double  indirect blocks
    if (it->nth == double_start)
      get_block(me, i_block[13], it->map2);
    if ((it->nth - double_start) % blks_per == 0)
      get_block(me, it->map2[(it->nth - double_start) / blks_per], it->map1);
    get_block(me, it->map1[(it->nth - double_start) % blks_per], it->buf);
  } else if (it->nth < triple_end) {
    // triple  indirect blocks
    if (it->nth == triple_start)
      get_block(me, i_block[12], it->map3);
    if ((it->nth - triple_start) % (blks_per * blks_per) == 0)
      get_block(me, it->map3[(it->nth - triple_start) / (blks_per * blks_per)],
                it->map2);
    if ((it->nth - triple_start) % blks_per == 0)
      get_block(me, it->map2[(it->nth - triple_start) / blks_per], it->map1);
    get_block(me, it->map1[(it->nth - triple_start) % blks_per], it->buf);
  }
  return it->buf;
}
